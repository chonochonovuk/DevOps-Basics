...
docker swarm init --advertise-addr 192.168.89.100

echo "* Retrieve manager token ..."
echo $(docker swarm join-token -q worker) > /vagrant/worker

echo "* Add Docker Image needed in stack ..."
docker image load -i /vagrant/javaspring.tar
